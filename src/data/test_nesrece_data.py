...
print("CWD:", os.getcwd())
print("Reading CURRENT_PATH:", CURRENT_PATH)
...
for df in (current, reference):
    ...
    df["response_delay_s"] = (df["prijavaCas"] - df["nastanekCas"]).dt.total_seconds()

# Use only numeric derived columns for drift (stable and meaningful)
cols_to_drop = ["prijavaCas", "nastanekCas", "obcinaNaziv"]
ref_analysis = reference.drop(columns=cols_to_drop, errors="ignore")
cur_analysis = current.drop(columns=cols_to_drop, errors="ignore")
...
if not all_tests_passed:
    print("\nData drift tests FAILED. Reference data NOT updated.")
    sys.exit(1)

print("\nData drift tests PASSED. Promoting current data to reference.")
os.makedirs(os.path.dirname(REFERENCE_PATH), exist_ok=True)
current.to_csv(REFERENCE_PATH, index=False)
sys.exit(0)