...
def scale_and_window(df_split, scaler, window_size):
    ...
    for i in range(len(X_scaled) - window_size):
        ...
        y_list.append(y_labels[i + window_size])
...
with mlflow.start_run(run_name="train_nesrece"):
    ...
    # Check model is not NaN before evaluating
    test_pred = model.predict(X_train[:1], verbose=0)
    if np.isnan(test_pred).any():
        print("[ERROR] Model produces NaN — training failed. Exiting.")
        sys.exit(1)
    ...
    test_pred_full = model_full.predict(X_full[:1], verbose=0)
    if np.isnan(test_pred_full).any():
        print("[ERROR] Full model produces NaN — training failed. Exiting.")
        sys.exit(1)
    ...
    # Remove trailing output dimension: (500, 7, 17, 1) → (500, 7, 17)
    ...
    if shap_mean.shape[1] != len(ALL_FEATURES):
        shap_mean = shap_mean[:, :len(ALL_FEATURES)]

    feature_importance = pd.DataFrame({
        "feature":    ALL_FEATURES,
        "importance": np.nan_to_num(shap_mean.mean(axis=0)),
    ...
    for obcina in df_model["obcinaNaziv"].unique():
        ...
        if mask.sum() > 0:
            obcina_shap[obcina] = np.nan_to_num(
                shap_mean[mask.values].mean(axis=0)
            ).tolist()
